...
class FossilDex:
    def __init__(self, level_ref):
        ...
        # ── Load Graphics ────────────────────────────────────────────────────
        self.img_body = pygame.image.load(resource_path('graphics/fossildex/device/dexbody.png')).convert_alpha()
        self.img_cursor = pygame.image.load(resource_path('graphics/fossildex/device/dexcursor.png')).convert_alpha()
        
        self.infographics = []
        for i in range(1, 7):
            surf = pygame.image.load(resource_path(f'graphics/fossildex/infographics/fossil{i}.png')).convert_alpha()
            self.infographics.append(surf)

        # ── Fonts & Text ─────────────────────────────────────────────────────
        self.font = pygame.font.Font(resource_path('graphics/font/joystix.ttf'), 20)
        
        self.fossil_names = [
            "Trilobite", "Ammonite", "T. Rex", 
            "Mammoth", "Amber", "Archaeopteryx"
        ]
